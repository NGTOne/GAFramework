...
int main(void) {
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int k = 0; k < 4; k++) {
			...
			firstLevelPools[i][k] = new HierarchicalGenePool(4, templateIndividual, 100, 1, bottomLevelModels[i][k]);
		}
	}
	...
	for (int i = 0; i < 4; i++) {
		...
		secondLevelPools[i] = new HierarchicalGenePool(8, templateIndividual, 100, 1, secondLevelModels[i]);
	}
	...
	GenePool * topLevelPool = new HierarchicalGenePool(8, templateIndividual, 100, 1, topLevelModel);
